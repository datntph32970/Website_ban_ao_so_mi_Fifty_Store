...
namespace API.Services.Implementations
{
    public class SanPhamService : BaseService<SanPham>, ISanPhamService
    {
        ...
        public SanPhamService(IBaseRepository<SanPham> repository, IBaseRepository<SanPhamChiTiet> sanPhamChiTietRepository, IBaseRepository<GiamGia> giamGiaRepository, IBaseRepository<KhuyenMai> khuyenMaiRepository, IBaseRepository<HoaDon> hoaDonRepository) : base(repository)
        {
        ...
        public async Task<List<SanPhamAdminDTO>> GetAllSanPhamAdminDTOAsync()
        {
            try
            {
                ...
                List<SanPhamAdminDTO> sanPhamDTOs = result.Where(s => s != null).Select(s => new SanPhamAdminDTO
                {
                    id_san_pham = s.id_san_pham,
                    ma_san_pham = s.ma_san_pham,
                    ten_san_pham = s.ten_san_pham,
                    mo_ta = s.mo_ta,
                    url_anh_mac_dinh = s.anhMacDinh != null ? s.anhMacDinh.url : null,
                    ...
                    sanPhamChiTiets = s.SanPhamChiTiets.Select(spct => new SanPhamChiTietAdminDTO
                    {
        ...
        public async Task<SanPhamAdminDTO> GetByIdSanPhamAdminDTOAsync(Guid id)
        {
            ...
            SanPhamAdminDTO sanPhamDTO = new SanPhamAdminDTO
            {
                ...
                mo_ta = result.mo_ta,
                url_anh_mac_dinh = result.anhMacDinh != null ? result.anhMacDinh.url : null,
        ...
        public Task<bool> UpdateTrangThaiAsync(Guid id, string trangThai)
        {
            throw new NotImplementedException();
        }
        public async Task<List<SanPhamAdminDTO>> GetByConditionWithIncludeAsync(Expression<Func<SanPham, bool>> condition, params Expression<Func<SanPham, object>>[] includes)
        {
            var result = await _repository.GetAllWithIncludeAsync(q => q.Where(condition).Include(includes[0]));
            return result.Select(s => new SanPhamAdminDTO
            {
                id_san_pham = s.id_san_pham,
                ma_san_pham = s.ma_san_pham,
                ten_san_pham = s.ten_san_pham,
